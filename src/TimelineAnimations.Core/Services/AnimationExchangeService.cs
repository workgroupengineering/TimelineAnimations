...
public static partial class AnimationExchangeService
{
    ...
    private static string ExportAvaloniaXaml(TimelineDocument document)
    {
        ...
        canvas.Add(
            new XElement(
                ...
                new XElement(
                    s_avaloniaNs + "LinearGradientBrush",
                    new XAttribute("StartPoint", "0,0"),
                    new XAttribute("EndPoint", "1,1"),
                    new XElement(s_avaloniaNs + "GradientStop", new XAttribute("Color", scene.BackgroundFrom), new XAttribute("Offset", "0")),
                    new XElement(s_avaloniaNs + "GradientStop", new XAttribute("Color", scene.BackgroundTo), new XAttribute("Offset", "1")))));
        ...
        foreach (var layer in scene.Layers.OrderBy(layer => layer.ZIndex))
        {
            ...
            if (samples.Count > 1)
            {
                styles.Add(BuildAvaloniaAnimationStyle(elementName, samples, scene.Duration));
            }
        }
    ...
    private static string ExportSvg(TimelineDocument document)
    {
        ...
        root.Add(
            new XElement(
                s_svgNs + "defs",
                new XElement(
                    s_svgNs + "linearGradient",
                    new XAttribute("id", "stageGradient"),
                    new XAttribute("x1", "0%"),
                    new XAttribute("y1", "0%"),
                    new XAttribute("x2", "100%"),
                    new XAttribute("y2", "100%"),
                    new XElement(s_svgNs + "stop", new XAttribute("offset", "0%"), new XAttribute("stop-color", scene.BackgroundFrom)),
                    new XElement(s_svgNs + "stop", new XAttribute("offset", "100%"), new XAttribute("stop-color", scene.BackgroundTo)))));
        root.Add(
            new XElement(
                s_svgNs + "rect",
                new XAttribute("width", FormatNumber(scene.CanvasWidth)),
                new XAttribute("height", FormatNumber(scene.CanvasHeight)),
                new XAttribute("fill", "url(#stageGradient)")));
    ...
    private static string ExportHtml(TimelineDocument document)
    {
        ...
        var stage = new XElement(
            ...
            new XAttribute("style", $"width:{FormatNumber(scene.CanvasWidth)}px;height:{FormatNumber(scene.CanvasHeight)}px;background:linear-gradient(135deg,{scene.BackgroundFrom},{scene.BackgroundTo});"));
    ...
    private static AnimationExchangeImportResult ImportAvaloniaXaml(string content, string sourceLabel)
    {
        ...
        var scene = new SceneModel
        {
            ...
            CanvasWidth = width,
            CanvasHeight = height,
            BackgroundFrom = ExtractXamlBackground(surface, 0) ?? ExtractXamlBackground(root, 0) ?? "#09111F",
            BackgroundTo = ExtractXamlBackground(surface, 1) ?? ExtractXamlBackground(root, 1) ?? "#182748",
            FrameRate = 24d
        };
    ...
    private static AnimationExchangeImportResult ImportSvg(string content, string sourceLabel)
    {
        ...
        var backgroundRect = root.Elements().FirstOrDefault(element =>
            ...
            string.Equals(TryGetAttributeValue(element, "fill"), "url(#stageGradient)", StringComparison.OrdinalIgnoreCase));
        if (backgroundRect is not null)
        {
            var stops = root.Descendants().Where(element => element.Name.LocalName == "stop").ToList();
            if (stops.Count >= 2)
            {
                scene.BackgroundFrom = TryGetAttributeValue(stops[0], "stop-color") ?? scene.BackgroundFrom;
                scene.BackgroundTo = TryGetAttributeValue(stops[^1], "stop-color") ?? scene.BackgroundTo;
            }
        }

        foreach (var element in root.Elements().Where(IsImportableSvgElement))
        {
            if (element.Name.LocalName == "rect" && string.Equals(TryGetAttributeValue(element, "fill"), "url(#stageGradient)", StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
    ...
    private static AnimationExchangeImportResult ImportHtml(string content, string sourceLabel)
    {
        ...
        var scene = new SceneModel
        {
            ...
            CanvasWidth = width,
            CanvasHeight = height,
            BackgroundFrom = ExtractCssGradientColor(stageStyle, 0) ?? "#09111F",
            BackgroundTo = ExtractCssGradientColor(stageStyle, 1) ?? "#182748",
            FrameRate = 24d
        };
    ...
    private static XElement BuildAvaloniaAnimationStyle(string elementName, IReadOnlyList<LayerFrameSample> samples, double duration)
    {
        ...
        foreach (var sample in samples)
        {
            animation.Add(
                new XElement(
                    ...
                    new XAttribute("Cue", $"{sample.Cue * 100d:0.###}%"),
        ...
        return new XElement(
            s_avaloniaNs + "Style",
            new XAttribute("Selector", $"#{elementName}"),
            new XElement(
    ...
    private static Dictionary<string, AvaloniaStyleAnimation> BuildAvaloniaStyleMap(XDocument xaml)
    {
        ...
        foreach (var style in xaml.Descendants().Where(element => element.Name.LocalName == "Style"))
        {
            ...
            styles[selector[1..]] = new AvaloniaStyleAnimation(duration, keyframes);
        }
