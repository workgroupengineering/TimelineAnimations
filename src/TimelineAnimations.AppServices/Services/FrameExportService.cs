...
public static class FrameExportService
{
    ...
    public static RenderTargetBitmap RenderFrameBitmap(
        ...
        bool transparentBackground = false,
        bool useWorkArea = false,
        RenderingEngineKind? renderingEngine = null)
    {
        SceneEditingService.EnsureScenes(document);
        ...
        using var rendered = RenderSceneBitmap(document, sequenceFrame.Scene, sequenceFrame.LocalTime, transparentBackground, renderingEngine);
        ...
        using var nextBitmap = RenderSceneBitmap(document, sequenceFrame.NextScene, sequenceFrame.NextLocalTime, transparentBackground, renderingEngine);
    ...
    private static WriteableBitmap RenderSceneBitmap(
        TimelineDocument document,
        SceneModel scene,
        double time,
        bool transparentBackground,
        RenderingEngineKind? renderingEngine)
    {
        var sceneDocument = BuildSceneDocument(document, scene);
        var request = RenderingEngineHostService.BuildRequest(
            sceneDocument,
            scene.Layers,
            time,
            scene.Duration,
            ...
            scene.CanvasWidth,
            scene.CanvasHeight,
            scene.TransparentStageBackground,
            scene.BackgroundFrom,
            scene.BackgroundTo);
        return renderingEngine.HasValue
            ? RenderingEngineHostService.RenderBitmap(request, renderingEngine.Value, includeBackground: !transparentBackground)
            : RenderingEngineHostService.RenderBitmap(request, includeBackground: !transparentBackground);
    }
